...
export default function Itp({ ssa, update, setUpdate, itpQs }) {
    const init = {}
    if (ssa.Itp.length === 0) {
        itpQs.map((q, index) => {
            init[index] = 0;
        ...
        itpQs.map((q, index) => {
            init[index] = Number(ssa.Itp[index])
        });
    }
    ...
    const handleChange = (e) => {
        ...
        setUpdate({ ...update, Itp: [...Object.values(form)] })
    }
                            ...
                            <Box sx={{ width: 200 }}>
                                ...
                                {ssa.Itp.length === 0 ? (
                                    ...
                                    <Slider
                                        className='slider'
                                        aria-label='Temperature'
                                        defaultValue={Number(ssa.Itp[index])}
                                        color='secondary'
                                        valueLabelDisplay='on'
                                        onChange={handleChange}
                                        name={`${index}`}
