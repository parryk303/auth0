...
export default function SocKPI({ ssa, update, setUpdate, socKPIQs }) {
    const init = {}
    if (ssa.SocKPI.length === 0) {
        socKPIQs.map((q, index) => {
            init[index] = 0;
        });
    } else {
        socKPIQs.map((q, index) => {
            init[index] = Number(ssa.SocKPI[index])
        });
    }
    ...
    const handleChange = (e) => {
        ...
        setUpdate({ ...update, SocKPI: [...Object.values(form)] })
    }
                            ...
                            <Box sx={{ width: 200 }}>
                                ...
                                {ssa.SocKPI.length === 0 ? (
                                    ...
                                    <Slider
                                        className='slider'
                                        aria-label='Temperature'
                                        defaultValue={Number(ssa.SocKPI[index])}
                                        color='secondary'
                                        valueLabelDisplay='on'
                                        onChange={handleChange}
                                        name={`${index}`}
