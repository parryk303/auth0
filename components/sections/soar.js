...
export default function Soar({ ssa, update, setUpdate, soarQs }) {
    const init = {}
    if (ssa.Soar.length === 0) {
        soarQs.map((q, index) => {
            init[index] = 0;
        });
    } else {
        soarQs.map((q, index) => {
            init[index] = Number(ssa.Soar[index])
        });
    }
    ...
    const handleChange = (e) => {
        ...
        setUpdate({ ...update, Soar: [...Object.values(form)] })
    }
                            ...
                            <Box sx={{ width: 200 }}>
                                ...
                                {ssa.Soar.length === 0 ? (
                                    ...
                                    <Slider
                                        className='slider'
                                        aria-label='Temperature'
                                        defaultValue={Number(ssa.Soar[index])}
                                        color='secondary'
                                        valueLabelDisplay='on'
                                        onChange={handleChange}
                                        name={`${index}`}
