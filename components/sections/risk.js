...
export default function Risk({ ssa, update, setUpdate, riskQs }) {
    const init = {}
    if (ssa.Risk.length === 0) {
        riskQs.map((q, index) => {
            init[index] = 0;
        });
    } else {
        riskQs.map((q, index) => {
            init[index] = Number(ssa.Risk[index])
        });
    }
    ...
    const handleChange = (e) => {
        ...
        setUpdate({ ...update, Risk: [...Object.values(form)] })
    }
                            ...
                            <Box sx={{ width: 200 }}>
                                ...
                                {ssa.Risk.length === 0 ? (
                                    ...
                                    <Slider
                                        className='slider'
                                        aria-label='Temperature'
                                        defaultValue={Number(ssa.Risk[index])}
                                        color='secondary'
                                        valueLabelDisplay='on'
                                        onChange={handleChange}
                                        name={`${index}`}
