...
export default function OnCall({ ssa, update, setUpdate, onCallQs }) {
    const init = {}
    if (ssa.OnCall.length === 0) {
        onCallQs.map((q, index) => {
            init[index] = 0;
        });
    } else {
        onCallQs.map((q, index) => {
            init[index] = Number(ssa.OnCall[index])
        });
    }
    ...
    const handleChange = (e) => {
        ...
        setUpdate({ ...update, OnCall: [...Object.values(form)] })
    }
                            ...
                            <Box sx={{ width: 200 }}>
                                ...
                                {ssa.OnCall.length === 0 ? (
                                    ...
                                    <Slider
                                        className='slider'
                                        aria-label='Temperature'
                                        defaultValue={Number(ssa.OnCall[index])}
                                        color='secondary'
                                        valueLabelDisplay='on'
                                        onChange={handleChange}
                                        name={`${index}`}
