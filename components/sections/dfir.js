...
export default function Dfir({ ssa, update, setUpdate, dfirQs }) {
    const init = {}
    if (ssa.Dfir.length === 0) {
        dfirQs.map((q, index) => {
            init[index] = 0;
        });
    } else {
        dfirQs.map((q, index) => {
            init[index] = Number(ssa.Dfir[index])
        });
    }
    ...
    const handleChange = (e) => {
        ...
        setUpdate({ ...update, Dfir: [...Object.values(form)] })
    }
                            ...
                            <Box sx={{ width: 200 }}>
                                ...
                                {ssa.Dfir.length === 0 ? (
                                    ...
                                    <Slider
                                        className='slider'
                                        aria-label='Temperature'
                                        defaultValue={Number(ssa.Dfir[index])}
                                        color='secondary'
                                        valueLabelDisplay='on'
                                        onChange={handleChange}
                                        name={`${index}`}
